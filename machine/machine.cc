...
void Machine::tlbTrans(int va){

	unsigned int vpn = (unsigned) va / PageSize;
	unsigned int offset = (unsigned) va % PageSize;
	int smallTimes = 1<<30;
	int num = -1;
	...
	for (int i=0;i<TLBSize;i++){
		...
		if(!tlb[i].valid){
			...
			tlb[i].physicalPage = pageTable[vpn].physicalPage;
			tlb[i].readOnly = pageTable[vpn].readOnly;
		    tlb[i].use = pageTable[vpn].use;
		    tlb[i].dirty = pageTable[vpn].dirty;
			tlb[i].hittimes = 0;
			return;
		}
		
		if(tlb[i].hittimes < smallTimes){
			smallTimes = tlb[i].hittimes;
			num = i;
		}
	}
	
	tlb[num].virtualPage = vpn;
	tlb[num].physicalPage = pageTable[vpn].physicalPage;
    tlb[num].readOnly = pageTable[vpn].readOnly;
    tlb[num].use = pageTable[vpn].use;
    tlb[num].dirty = pageTable[vpn].dirty;
	tlb[num].hittimes = 0;
}